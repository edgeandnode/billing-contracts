...
import { PaymentMockNoTransferOnCreate } from '../../../build/types'
...
describe('RecurringPayments: payment types', () => {
  ...
  let payment: PaymentMockNoTransferOnCreate
  ...
  beforeEach(async function () {
    ...
    payment = await deployment.deployPaymentMockNoTransferOnCreate([token.address], me.signer, true)
  ...
  describe('Payment type: Payment mock no transfer', function () {
    ...
    describe('create()', function () {
      it('should revert if RP contract balance changes unexpectedly', async function () {
        ...
        // Create RP
        await token.connect(user1.signer).approve(recurringPayments.address, ten.add(initialAmount))
        const tx = recurringPayments
          .connect(user1.signer)
          .create(paymentTypeName, initialAmount, recurringAmount, ten, createData)
