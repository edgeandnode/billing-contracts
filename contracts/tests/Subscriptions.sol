...
contract Subscriptions is Ownable {
    ...
    event Subscribe(address indexed user, uint256 indexed epoch, uint64 start, uint64 end, uint128 rate);
    ...
    event TokensCollected(address indexed owner, uint256 amount, uint256 indexed startEpoch, uint256 indexed endEpoch);
    event RecurringPaymentsUpdated(address indexed recurringPayments);

    modifier onlyRecurringPayments() {
        require(msg.sender == recurringPayments, "caller is not the recurring payments contract");
        _;
    }
    ...
    constructor(address _token, uint64 _epochSeconds, address _recurringPayments) {
        token = IERC20(_token);
    ...
    function setPendingSubscription(uint64 start, uint64 end, uint128 rate) public {
    ...
    function fulfil(address _to, uint256 _amount) public {
        ...
        require(pendingSub.start != 0 && pendingSub.end != 0, "No pending subscription");
        ...
        if (extra > 0) {
            bool pullSuccess = token.transferFrom(msg.sender, address(this), extra);
    ...
    function locked(uint64 _subStart, uint64 _subEnd, uint128 _subRate) public view returns (uint128) {
        uint256 len = uint256(SignedMath.max(0, int256(Math.min(block.timestamp, _subEnd)) - int64(_subStart)));
    ...
    function unlocked(uint64 _subStart, uint64 _subEnd, uint128 _subRate) public view returns (uint128) {
        uint256 len = uint256(SignedMath.max(0, int256(int64(_subEnd)) - int256(Math.max(block.timestamp, _subStart))));
        return _subRate * uint128(len);
    }
